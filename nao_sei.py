...
def evaluate_network(dropout, normalization, learning_rate, neurons,optimizer, epochs, 
    ...
    for train_index, test_index in kf.split(x_train):
        ...
        model.fit(xtr, ytr, epochs=epochs, batch_size=batch_size, verbose=1)

        y_pred = model.predict(xte)
        y_pred = (np.asarray(y_pred)).round()
        y_pred = y_pred.astype(int)
        # y_pred = pd.DataFrame(y_pred, columns = [3,4,5,6,7,8,9])
        yte = np.array(yte)
